...
description: Guidelines for working with MagicPath through the MagicPath MCP server to find, preview, inspect, install, export, create, and edit UI components and manage MagicPath skills. Use for MagicPath designs; projects and canvases; selected components, images, or revisions; themes and design systems; teams, members, ownership, and attribution; installed-component audits; and share links. Also use when exporting exact components or revisions to local apps; replacing, adapting, or translating local UI with 1:1 fidelity and only requested changes; installing React/TypeScript components from fetched source; authoring responsive interactive canvas components; importing UI from local paths or Git repositories into MagicPath; or creating, retrieving, updating, importing, enabling, disabling, or deleting MagicPath skills. In hosts with an embedded browser, keep the project canvas open through share URLs for visual work.
