...
Use this reference when MagicPath runs through an agent host that exposes an embedded browser, browser pane, or webview, such as Codex or Cursor when that capability is available.
...
## Resolve a project URL

The `magicpath` MCP server never opens a browser — it is remote. To show a project, call the `share` capability for the project ID. The response includes the canonical URL:
...
When no embedded browser is available, it cannot be controlled reliably, or the user explicitly wants their normal browser, give the user the `share` URL to open themselves.

## Browser sign-in is separate from MCP sign-in

The MCP server can be authorized while the embedded browser is not signed in. `whoami`, `create-project`, or `code submit` succeeding does not prove that the browser pane has a MagicPath login session.

The project URL returned by `share` is the intended canvas URL. If opening `/files/<projectId>` in the embedded browser redirects to the MagicPath home or sign-in flow:
...
The reverse also holds: being signed into MagicPath in a browser does not authorize the MCP server. If tool calls fail with authorization errors, the user must authenticate the `magicpath` server through the client's MCP authorization flow.

## Project-canvas workflows

### Work in the active project

1. If the user refers to the project they have open, call `active-project`.
2. If it resolves to one project and it is not already visible in the embedded browser, call `share` for it and open the returned project URL there.
...
1. Create the project with `create-project`.
2. Read the returned `project.id`.
3. **Before authoring a design**, in an embedded-browser host call `share` for the project and open the returned URL in the internal browser.
...
5. If no embedded browser is available, give the user the URL only when user-facing project navigation is needed.
6. Continue the requested canvas workflow with `code start` against the project, keeping the project canvas visible for review and selection.
...
1. If the user asks for a link to an individual design, resolve it with `share` for the component's `generatedName` and return its `url`.
...
- Calling `whoami`, `list-projects`, `list-components`, `list-teams`, `list-members`, `search`, `list-themes`, or `get-theme` only to make an internal decision.
- Fetching component source or installing a component where the requested result is repository code rather than visible canvas work.
- Running automated checks or polling a `code status` job before the project contains a viewable result.
- The project canvas is already visible and the next step does not require navigation.
...
- **No embedded browser:** Continue with the MCP tools and give the user `share` URLs for intentional project navigation.

## Boundaries

- Use the main `https://www.magicpath.ai` product surface returned by `share`.
- Do not interfere with the client's MCP authorization flow; authentication is owned by the client.
- Do not confuse MCP authorization with the login session in the embedded browser pane — they are separate.
