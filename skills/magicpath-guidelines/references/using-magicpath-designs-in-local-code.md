...
- If the user refers to "this," "the selected design," or a component visible on the canvas, call `selection`. Use the selected entry's `id`, `generatedName`, `projectId`, and `selectedRevisionId` from the returned `components` array. A canvas selection is already an explicit choice; do not search for a different component.
...
- If several components are selected, determine whether they are independent exports or parts of one deliverable. Stage each independent component in its own directory so their `src/App.tsx` and `src/index.css` files cannot overwrite one another.
...
1. Fetch the full source and read it — every file, dependency, import, asset, and assumption about layout or theming.
2. Check the target paths under `src/components/magicpath/<name>/` for existing files; review any collisions before overwriting.
3. Write the files, then install the returned dependencies with the project's package manager.
...
Never install MagicPath component files into a non-React project. Pull the exact source with the revision-aware fetch when a component ID is available, or the registry fetch for a component known only by `generatedName`. Translate rendered structure and behavior into the target framework while preserving concrete values. Do not reinterpret the design using the target platform's default components or theme.
...
- Recreating from a screenshot instead of fetching the source.
- Copying one TSX file while omitting generated subcomponents, `index.css`, tokens, fonts, or assets.
- Treating exported Component Forge source as a complete runnable project.
- Running `code start` and creating canvas edit state for a read-only export.
- Writing exported files into an existing app root and overwriting entry/style files.
...
- The component is actually imported/rendered when it was installed for use.
