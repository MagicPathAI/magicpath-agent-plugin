...
> **IMPORTANT:** This flow is the **inverse** of installing MagicPath components. When installing, the source of truth is the MagicPath platform and the destination is the user's app. Here the **source of truth is the user's repository** and the **destination is the MagicPath canvas**. You recreate the repo's UI as a canvas component using the `code start` → `code submit` authoring flow. Do **not** use the source-fetch/install capabilities for this — they serve the other direction (see the boundaries section below).
>
...
If the user is instead asking to install a MagicPath component into their app, that is the install flow — not this one.
...
Keep the cloned repo and your authoring working directory **separate**. The repo is read-only input; the working directory is where you draft the recreation before submitting it through the authoring tools.
...
> **If the user has a MagicPath theme that corresponds to this brand**, prefer reconciling against it: call `list-themes` / `get-theme` and use the theme's CSS variables, fonts, and `prompt` as the styling target. Otherwise, derive the tokens straight from the repo as above.
...
3. Decide scope against **Design Default rule 5** (never stack multiple screens in one frame):
   ...
   - Several genuinely independent screens → **separate** canvas components, one per screen, each authored in its own session with its own working directory.
   Confirm the split with the user before building many frames.

## Phase 3 — Plan the recreation

1. **Choose the frame(s)** and canvas dimensions from the source's intended viewport — e.g. `1440×900` for a desktop app, `390×844` for a mobile screen. The content stays fluid; the dimensions just signal the target form factor.
...
4. **Plan assets:** real images are staged as `assets/` files in the authoring session and referenced from code/CSS. Do **not** hotlink repo blob URLs and do **not** inline `data:image/...;base64,...`.
...
7. **Submit and fix:** call `code submit` with the dimensions and wait for the build. If the job fails, read the returned diagnostics, fix only the allowed files, and re-submit. Don't start a new component to work around a build failure.

## Phase 5 — Verify fidelity

- Open the result on the project canvas (or via its `share` URL) and compare it against the source app side by side.
...
| Real images / SVGs | Stage in `assets/` and reference; never base64-inline or hotlink repo blobs |
...
- **Don't use the source-fetch/install capabilities.** They read MagicPath-side source for installing into an app; that's the opposite direction. Here you're reading the **user's repo** and creating a **new** canvas component via `code start` → `code submit`.
- **Don't submit forbidden files**: no `package.json`, `vite.config.*`, `src/main.tsx`, lockfiles, or arbitrary repo files — only `src/App.tsx`, `src/index.css`, `src/components/generated/**`, and `assets/**`.
...
**"Bring the sidebar of my app into MagicPath"**
1. Get the code (Phase 0). 2. Read global CSS + tokens + fonts (Phase 1). 3. Open the `Sidebar` component, follow its imports (child items, icons, styles) and read its layout parent for width/position (Phase 2). 4. `code start` against the project with the name "Sidebar" and suitable dimensions. 5. Recreate it faithfully in `src/components/generated/Sidebar.tsx` — real collapse/active-item state, matched colors/spacing/typography, fluid + centered, no device frame. 6. `code submit` and wait. 7. Verify against the app.

**"Render this project / this page in MagicPath"**
1. Get the code (Phase 0). 2. Read the design foundation (Phase 1). 3. Identify the page entry; if ambiguous, ask which screen and **stop**. Walk the tree and decide one interactive frame vs. multiple frames (Phase 2 / Design Default rule 5). 4. For each frame: `code start` with viewport-appropriate dimensions, recreate faithfully with internal navigation state for multi-view flows, mock data realistically. 5. `code submit` each and wait. 6. Verify each against the source.
